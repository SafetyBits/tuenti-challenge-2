...
// I had an error, this function returned "long" instead of "unsigned long long".
unsigned long long solve(int n_cards, int split) {
	int len_first_deck = split;
	int len_second_deck = n_cards - split;
	
	// unshuffle the deck (rollback, if you prefer)
	...
	// now cards[0] contains the location of card "0" after a shuffle, and so on
